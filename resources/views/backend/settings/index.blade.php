            ...
            @foreach($settings['system'] as $group)
                <div role="tabpanel" class="tab-pane" id="group_{{ $group['name'] }}">
                    @foreach($group['settings'] as $setting)
                        ...
                        @elseif ($setting['type'] == 'mailinglist')
                            {{ Form::bsSelect('settings_'.$group['name'].'_'.$setting['name'], $mailinglists, settings($group['name'].'.'.$setting['name']), ['data-live-search' => 'true', 'data-size' => 5], trans($setting['trans'])) }}
